...
- MCP server exposes the full pipeline as tools Claude can call directly.
...
- The first run will auto-build a local SQLite DB at `mimic_baseline1/data/mimic_demo.db` from the CSVs.
- No need to set `MIMIC_DSN`.

Run (structured args)
1) Configure stats in `conf/stats.yaml` (already includes heart_rate).
2) Example usage:
   - `python -m mimic_baseline1.src.run_example 10006 2111-10-20 heart_rate`
...
- Ask a question such as:
  - `python -m mimic_baseline1.src.run_question "How is 10011 doing on 2126-08-20?"`
...
- Exposes 5 tools Claude can call directly:
  ...
  | Tool | Description |
  |------|-------------|
  | `query_patient_data` | Fetch raw vital sign rows for a patient on a given day |
  | `compute_features` | Compute statistical features (range, trend, outliers, etc.) |
  | `generate_summary` | LLM-generated narrative summary of vital signs |
  | `query_guidelines` | RAG lookup of clinical guidelines relevant to the patient's vitals |
  | `analyze_patient` | Full pipeline: fetch → features → summary → guidelines in one call |

- All tools accept `subject_id` (int), `day` (YYYY-MM-DD), and `stat_key` (default: `"heart_rate"`).
- `query_guidelines` also accepts an optional `question` string for targeted guideline lookup.
- Requires `OPENAI_API_KEY` for `generate_summary`, `query_guidelines`, and `analyze_patient`.

Benchmark the SQL agent
- Define a list of benchmark cases in `conf/sql_benchmark_cases.yaml` (subject_id, day, stat_key).
- Run `python -m mimic_baseline1.src.benchmark_sql --cases mimic_baseline1/conf/sql_benchmark_cases.yaml`.
...
- src/guideline_rag.py        # RAG over clinical guideline docs
